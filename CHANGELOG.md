...
- Recovery - startup recovery opts out of rate limiting explicitly, so a globally configured limiter no longer logs a "no rules" warning per recovered agent; [`Legion.resume/2`](https://hexdocs.pm/legion/Legion.html#resume/2) and [`Legion.recover/2`](https://hexdocs.pm/legion/Legion.html#recover/2) document that rules are not persisted
